...
# Add src directory to path - must be done before importing from src
src_path = Path(__file__).parent / "src"
sys.path.insert(0, str(src_path))

# ruff: noqa: E402 - imports after path modification is intentional
...
def test_batch_embeddings():
    ...
    for i, embedding in enumerate(embeddings):
        assert len(embedding) == 1024, (
            f"Embedding {i} has wrong dimensions: {len(embedding)}"
        )
    ...
    for i, (text, embedding) in enumerate(zip(texts, embeddings)):
        if text.strip():  # Non-empty text
            ...
            assert non_zero_count > 0, (
                f"Non-empty text {i} should have non-zero embedding"
            )
...
def test_similarity():
    ...
    similar_texts = ["The cat sat on the mat.", "A feline rested on the rug."]
    ...
    assert similar_score > different_score, (
        f"Similar texts ({similar_score}) should be more similar than different texts ({different_score})"
    )
